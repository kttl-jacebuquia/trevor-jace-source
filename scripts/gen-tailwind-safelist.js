...
function optionWalker(root, prefix = '', walk = 2) {
	...
	Object.keys(root).forEach(key => {
		...
		out[key] = key + ': ' + ((/boolean|number|string/).test(typeof val) ? val : JSON.stringify(val));
	})
...
options['text'] = Object.assign(optionWalker(theme.textColor), optionWalker(theme.fontSize));
...
Object.keys(options).forEach(rule => {
	...
	['', 'md', 'xl'].forEach(screen => Object.keys(val).forEach(entry => fileContent.push(`${screen + (screen && ':')}${rule}-${entry}`)))
})
