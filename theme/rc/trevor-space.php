...
<main id="site-content" role="main" class="site-content">
	<div>
		<div class="container mx-auto text-center site-content-inner">
			...
			<h1 class="heading-lg-tilted text-indigo mb-4"><?= Trevorspace::get_val( Trevorspace::SETTING_HOME_TITLE ) ?></h1>
			<p class="font-normal text-px18 mb-7 text-indigo md:text-px20 md:leading-px30 md:mx-px88 md:mb-5 lg:text-px22 lg:leading-px36 lg:mb-14 lg:mx-64"><?= Trevorspace::get_val( Trevorspace::SETTING_HOME_DESC ) ?></p>

			<a href="<?= Trevorspace::get_val( Trevorspace::SETTING_HOME_JOIN_URL ) ?>"
			   ...
			   class="block text-white font-bold text-px18 leading-px24 bg-indigo py-4 px-8 rounded-px10 mx-auto md:text-px16 md:leading-px22 lg:mb-9 lg:text-px18 lg:leading-px22 lg:px-20 lg:py-6">
				<?= Trevorspace::get_val( Trevorspace::SETTING_HOME_JOIN_CTA ) ?>
			</a>
			<div class="login-row mt-5 mb-20 text-indigo md:mb-28 lg:mb-40">
				Already member?
				<a href="<?= Trevorspace::get_val( Trevorspace::SETTING_HOME_LOGIN_URL ) ?>"
				   rel="noreferrer noopener" target="_blank" class="btn-link">
					<?= Trevorspace::get_val( Trevorspace::SETTING_HOME_LOGIN_CTA ) ?>
				</a>
			</div>
			...
			<?php if ( ! empty( $page_data[0] ) ): ?>
				<div class="flex flex-col mb-14 md:flex-row lg:flex-row-reverse lg:mx-px106 lg:mb-28">
					<div class="bg-white rounded-px10 mb-7 md:flex-1 md:mb-0" data-aspectRatio="1:1">
						<?php if ( ! empty( $page_data[0]['img'] ) ) {
							echo wp_get_attachment_image( $page_data[0]['img']['id'], 'medium', false, [
									'class' => implode( ' ', [
											'object-center',
											'object-cover',
											'rounded-px10',
			...
			<?php if ( ! empty( $page_data[1] ) ): ?>
				<div class="flex flex-col mb-14 md:flex-row-reverse lg:flex-row lg:mx-px106 lg:mb-28">
					<div class="bg-white rounded-px10 mb-7 md:flex-1 md:mb-0" data-aspectRatio="1:1">
						<?php if ( ! empty( $page_data[1]['img'] ) ) {
							echo wp_get_attachment_image( $page_data[1]['img']['id'], 'medium', false, [
									'class' => implode( ' ', [
											'object-center',
											'object-cover',
											'rounded-px10',
			...
			<?php if ( ! empty( $page_data[2] ) ): ?>
				<div class="flex flex-col mb-20 md:flex-row md:mb-28 lg:flex-row-reverse lg:mx-px106">
					<div class="bg-white rounded-px10 mb-7 md:flex-1 md:mb-0" data-aspectRatio="1:1">
						<?php if ( ! empty( $page_data[2]['img'] ) ) {
							echo wp_get_attachment_image( $page_data[2]['img']['id'], 'medium', false, [
									'class' => implode( ' ', [
											'object-center',
											'object-cover',
											'rounded-px10',
