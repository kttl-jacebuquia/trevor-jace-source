...
class Trevorspace extends Abstract_Customizer {
	...
	protected function _register_panels(): void {
		$this->_manager->add_panel( self::PANEL_ID, [ 'title' => 'Trevorspace' ] );
	}

	/** @inheritDoc */
	protected function _register_sections(): void {
		$this->_manager->add_section( self::SECTION_COUNTER, [
			'panel' => self::PANEL_ID,
			'title' => 'Online Count',
		] );
	...
	protected function _register_controls(): void {
		/* * Counter */
		$this->_manager->add_control( self::SETTING_COUNTER_URL, [
			'setting' => self::SETTING_COUNTER_URL,
			'section' => self::SECTION_COUNTER,
			'label'   => 'URL',
			'type'    => 'url',
		] );
		$this->_manager->add_control( self::SETTING_COUNTER_ONLINE_TXT, [
			'setting' => self::SETTING_COUNTER_ONLINE_TXT,
			'section' => self::SECTION_COUNTER,
			'label'   => 'Online Text',
			'type'    => 'text',
		] );
		$this->_manager->add_control( self::SETTING_COUNTER_ONLINE_THRESHOLD, [
			'setting' => self::SETTING_COUNTER_ONLINE_THRESHOLD,
			'section' => self::SECTION_COUNTER,
			'label'   => 'Threshold',
			'type'    => 'number',
		] );
		$this->_manager->add_control( self::SETTING_COUNTER_UNDER_THRESHOLD_MSG, [
		...
		$this->_manager->add_control( self::SETTING_HOME_JOIN_CTA, [
			'setting' => self::SETTING_HOME_JOIN_CTA,
			'section' => self::SECTION_HOME,
			'label'   => 'Join CTA',
			'type'    => 'text',
		] );
		$this->_manager->add_control( self::SETTING_HOME_JOIN_URL, [
			'setting' => self::SETTING_HOME_JOIN_URL,
			'section' => self::SECTION_HOME,
			'label'   => 'Join Url',
			'type'    => 'url',
		] );
		$this->_manager->add_control( self::SETTING_HOME_LOGIN_CTA, [
			'setting' => self::SETTING_HOME_LOGIN_CTA,
			'section' => self::SECTION_HOME,
			'label'   => 'Login CTA',
			'type'    => 'text',
		] );
		$this->_manager->add_control( self::SETTING_HOME_LOGIN_URL, [
			'setting' => self::SETTING_HOME_LOGIN_URL,
			'section' => self::SECTION_HOME,
			'label'   => 'Login Url',
			'type'    => 'url',
		] );
		$this->_manager->add_control( new Control\Custom_List( $this->_manager, self::SETTING_HOME_DATA, [
