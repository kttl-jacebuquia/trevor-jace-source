...
class Topic_Cards extends A_Field_Group implements I_Block, I_Renderable {
	...
	public static function register(): bool {
		...
		add_filter( 'acf/fields/relationship/result/name=' . static::FIELD_PRODUCTS, array( static::class, 'relationship_result_products' ), 10, 4 );
	 ...
	 */
	public static function relationship_result_products( $text, $post ) {
		$current_date = current_datetime();
		$current_date = wp_date( 'Ymd', $current_date->date, $current_date->timezone );
		...
		if ( strtotime( $current_date ) < strtotime( $start_date ) && strtotime( $current_date ) <= strtotime( $end_date ) ) {
			return "<em>Upcoming | $start_date<br/>$text</em>";
		} elseif ( strtotime( $current_date ) >= strtotime( $start_date ) && strtotime( $current_date ) <= strtotime( $end_date ) ) {
			// Active
			return $text;
		}
	}
