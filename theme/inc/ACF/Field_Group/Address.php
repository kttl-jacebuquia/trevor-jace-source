		...
		return [
				...
				'fields' => [
						...
						static::FIELD_ENTRIES => [
								...
								'layout'     => 'block',
								'sub_fields' => [
										static::FIELD_ENTRY_TITLE => [
												'key'   => $entry_title,
												'name'  => static::FIELD_ENTRY_TITLE,
												'label' => 'Title',
												'type'  => 'text',
										],
										static::FIELD_ENTRY_LINES => [
												'key'        => $entry_lines,
				...
				<div class="flex flex-col md:flex-row address-container mt-px20 lg:mt-px30">
					<?php foreach ( $entries as $entry ) { ?>
						<div class="mb-px28 md:mb-0 md:mr-px40 lg:mr-px50 md:mt-px40 lg:mt-0">
							<h3 class="font-bold mb-px6 lg:mb-px12 text-px20 leading-px30 md:leading-px26 lg:text-px24 lg:leading-px30"><?= esc_html( @$entry[ static::FIELD_ENTRY_TITLE ] ) ?></h3>
							<?php foreach ( @$entry['entry_lines'] as $line ) { ?>
								<div class="font-normal text-px18 leading-px24 lg:text-px22 lg:leading-px28"><?= esc_html( $line[ static::FIELD_LINE ] ); ?></div>
							<?php } ?>
						</div>
					<?php } ?>
				</div>
